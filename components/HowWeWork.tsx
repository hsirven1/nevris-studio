...
/**
 * Chapter title + studio principles — same editorial mark as Discover our work.
 */
export function HowWeWork() {
  ...
  return (
    ...
    >
      ...
      <div
        className="h-5 bg-ground md:h-[clamp(1.25rem,3.5vh,2.25rem)]"
        aria-hidden
      />

      <div className="gutter-x relative z-10 pb-16 md:pb-[clamp(3.75rem,9vh,5.5rem)]">
        <h3 className="font-display m-0 max-w-[16ch] text-[1.85rem] leading-[1.05] font-bold tracking-[-0.03em] text-ink md:max-w-[18ch] md:text-[clamp(1.85rem,3.8vw,2.75rem)] md:leading-[1.02]">
          {howWeWork.headline}
        </h3>
        ...
        <ol className="mt-8 mb-0 grid list-none grid-cols-1 gap-7 p-0 md:mt-[clamp(2rem,5vh,3rem)] md:grid-cols-2 md:gap-x-10 md:gap-y-12 lg:grid-cols-3 lg:gap-x-12 xl:gap-x-16">
          {howWeWork.principles.map((principle) => (
            <li key={principle.id} className="min-w-0">
              <p className="m-0 font-mono text-[2rem] leading-none tracking-[-0.05em] text-slate md:text-[clamp(2.75rem,5vw,4rem)]">
                {principle.id}
              </p>
