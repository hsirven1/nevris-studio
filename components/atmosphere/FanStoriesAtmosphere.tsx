...
/**
 * FanStories — soft lavender + light panel planes + quiet sport accent.
 * Same family as Juno: 2–3 rounded blocks, low opacity, one contour.
 */
export function FanStoriesAtmosphere() {
  ...
  const planeRotate = useTransform(p, [0, 1], [3, 7]);
  ...
  return (
    <div
      ...
      aria-hidden
    >
      {/* Lavender mass — frames the product column */}
      <AmbientDrift
        amplitude={12}
        duration={26}
      ...
      >
        <motion.div
          className="h-full w-full rounded-[2rem] bg-[rgba(201,182,247,0.34)] max-md:opacity-70"
          style={
            reduce
              ? undefined
              : {
      ...
      >
        <motion.div
          className="h-full w-full rounded-[2rem] bg-[rgba(255,255,255,0.42)]"
          style={reduce ? undefined : { x: coolX }}
        />
      </AmbientDrift>

      {/* Quiet sport accent — barely there */}
      <AmbientDrift
        amplitude={7}
        duration={32}
        delay={0.5}
      ...
      >
        <div className="h-full w-full rounded-full bg-[rgba(147,224,60,0.14)] blur-2xl" />
      </AmbientDrift>
